...
export class FarmingRenderer {
  ...
  static createIconCard(w, status) {
    ...
    let cardClass =
      "border-slate-700 bg-slate-800/50 opacity-50 grayscale hover:opacity-80";
    let icon = "";

    if (status === 1) {
      cardClass =
        "border-orange-500 bg-orange-900/30 ring-2 ring-orange-500/50 opacity-100";
      icon =
        '<div class="absolute top-1 right-1 bg-orange-500 text-white rounded-sm px-1.5 h-5 flex items-center justify-center text-[9px] shadow-md z-10 font-black">TGT</div>';
    } else if (status === 2) {
      cardClass =
        "border-emerald-500 bg-emerald-900/30 ring-2 ring-emerald-500/50 opacity-100";
      icon =
        '<div class="absolute top-1 right-1 bg-emerald-500 text-white rounded-sm px-1.5 h-5 flex items-center justify-center text-[9px] shadow-md z-10 font-black">OK</div>';
    }
  ...
  static renderDrawer(plan, currentIndex, totalPlans, priorityWeapon) {
    ...
    document.getElementById("loc-nav-indicator").innerText =
    ...
    if (plan.count === 0)
      descHtml = `<span class="text-slate-500">획득 가능 아이템 없음</span>`;
    ...
    const getTagStyle = (tag) => {
      ...
      if (APP_CONFIG.CATEGORIES.SERIES.includes(tag))
        return "bg-purple-600 text-white border-purple-500 shadow-md";
      return "bg-emerald-600 text-white border-emerald-500 shadow-md";
    };

    document.getElementById("farm-rec-main").innerHTML =
      plan.recommendStats.length > 0
        ? plan.recommendStats
            .map(
              (s) =>
                `<span class="${getTagStyle(s)} px-3 py-1.5 rounded-lg border text-xs font-bold transition-all whitespace-nowrap">${s}</span>`,
            )
            .join("")
        : "-";
  ...
  static renderDrawerList(plan, priorityWeapon) {
    ...
    container.className =
      "flex flex-nowrap gap-3 overflow-x-auto pb-8 mt-4 pt-4 border-t border-white/5 scrollbar-hide px-1";
